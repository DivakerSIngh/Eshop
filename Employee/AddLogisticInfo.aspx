<%@ Page Title="Logistic Info" Language="C#" MasterPageFile="~/Employee/EmployeePanel.master" AutoEventWireup="true" CodeFile="AddLogisticInfo.aspx.cs" Inherits="Logistic_AddLogisticInfo" %>
            ...
            $scope.add = function () {
                ...
                common.httpPost("AddLogisticInfo.aspx/addLogistic", "{'mobile':" + $scope.retailer.Mobile + ",'loginId':" + $('#hdnLoginId').val() + "}", false, success = function (data) {
                    if ($scope.retailerList.length == JSON.parse(data).length) {
                        ...
                        toastr.success('Logistic added successfull !');
                    }
                    $scope.retailerList = JSON.parse(data);
                    $scope.loading = false;
                }, failure = function (response) { });
